  ...
  return (
    <div data-testid="salary-page">
      ...
      {d && (<>
        ...
        <p className="text-xs text-muted-foreground mb-4">Daily allowance Rp 20.000 for each day with more than 20 cups sold. Period follows the 16th → 15th payroll cycle.</p>
          ...
          {d.riders.map((r) => (
            <Bento key={r.rider_id} className="fade-up" testId={`salary-card-${r.rider_id}`}>
              <div className="flex items-center gap-3 mb-4"><div className="w-11 h-11 rounded-full bg-primary/15 text-primary font-bold flex items-center justify-center overflow-hidden">{r.photo ? <img src={r.photo} alt="" className="w-full h-full object-cover" /> : initials(r.rider_name)}</div>
                <div className="flex-1"><p className="font-heading font-bold">{r.rider_name}</p><p className="text-xs text-muted-foreground">{r.cups} cups · {r.attendance_days} days attended</p></div>
                <div className="text-right"><p className="eyebrow">Total income</p><p className="num font-bold text-primary">{fmtRp(r.total_income)}</p></div></div>
              <div className="grid grid-cols-3 gap-2 text-center mb-4">
                ...
                <div className="rounded-xl bg-muted p-2"><p className="eyebrow">Allowance</p><p className="num text-xs font-bold">{fmtRp(r.allowance)}</p><p className="text-[10px] text-muted-foreground">{r.allowance_days} days</p></div>
